package app.burrow.features.account.chat.topic
...
import app.burrow.features.account.chat.ChatUser
...
data class Topic(
    ...
    companion object {
        /**
         * Convert a [row] to a [Topic].
         *
         * @param row The row containing a [Topic].
         */
        fun fromRow(row: ResultRow): Topic =
            Topic(
                id = row[Topics.id],
                name = row[Topics.name],
                description = row[Topics.description],
                createdBy = row[Topics.createdBy],
                createdAt = row[Topics.createdAt],
                pinned = row[Topics.pinned],
                expiresAt = row[Topics.expiresAt],
            )
    }
}
