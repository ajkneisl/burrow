package app.burrow.features.account.block
...
private suspend fun leaveBurrowsHostedBy(userID: String, hostUserID: String) = query {
    ...
    val activeBurrowIds =
        Burrows.innerJoin(membership.Memberships, { Burrows.id }, { membership.Memberships.burrowID })
            .select(Burrows.id)
            .where {
                (Burrows.ownerID eq hostUserID) and
                    (Burrows.endTime greater currentTime) and
                    (membership.Memberships.userID eq userID) and
                    (membership.Memberships.status eq BurrowMemberStatus.JOINED) and
                    (membership.Memberships.role neq BurrowRole.HOST)
            }
            .map { it[Burrows.id] }
            .toSet()
    ...
    membership.Memberships.update(
        where = {
            (membership.Memberships.userID eq userID) and (membership.Memberships.burrowID inList activeBurrowIds)
        }
    ) {
